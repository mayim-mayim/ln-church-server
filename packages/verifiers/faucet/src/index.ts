...
export class FaucetVerifier implements PaymentVerifier {
    ...
    async verify(req: Request, body: any): Promise<VerifyResult> {
        try {
            const token = body?.paymentOverride?.proof || body?.paymentAuth?.proof;
            if (!token) throw new Error("Faucet proof is missing.");
            ...
            const isValid = await crypto.subtle.verify(
                'HMAC',
                key,
                this.base64UrlToUint8Array(signatureB64).buffer as ArrayBuffer,
